...
public abstract class HtmlBase<T extends HtmlBase<?>> extends HtmlCore<T> {
	...
	public HtmlBase(Writer writer) {
		super(writer);
	}

	public HtmlBase(OutputStream out) {
		super(out);
	}
	...
	public T css(String selector, Object... styleValuePairs) {
		if (inCss) {
			throw new HyperMlException("Nested css() calls are not allowed");
		}
	...
	private Object[] mergeUnits(Object[] pairs) {
		...
		for (int i = 0, n = pairs.length; i < n; i += 2) {
			result.add(pairs[i]);
			if (i + 1 >= n) {
				throw new HyperMlException("Pairs array length must be even");
			}
